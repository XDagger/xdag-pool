...
import (
	...
	"github.com/XDagger/xdagpool/xdago/utils"
...
func (w *Wallet) UnlockWallet(password string) bool {
	...
	if len(password) == 0 {
		util.Error.Fatal("password can not be null")
	}

	if w.Exists() {
		data, err := os.ReadFile(w.file)
		if err != nil {
			util.Error.Fatal("read wallet file failed," + err.Error())
		...
		case 4:
			salt := readBytes(r)
			if r.Error() != nil {
				util.Error.Fatal("parse wallet salt failed," + r.Error().Error())

			}
			key, keyErr := cryptography.GenerateFromPassword(salt, []byte(password), BCRYPT_COST)
			if keyErr != nil {
				util.Error.Fatal("generate wallet Decrypt key failed," + err.Error())
			...
			if r.Error() != nil {
				util.Error.Fatal("parse wallet private keys failed," + r.Error().Error())
			}
		...
		default:
			util.Error.Fatal("wallet version error")
		}
	}
	w.password = password
	return true
...
func readAccounts(key []byte, r *utils.SimpleReader, vlq bool, version uint32) ([]*secp256k1.PrivateKey, error) {
	...
	for i := 0; i < int(total); i++ {
		...
		if err != nil {
			util.Error.Println("decrypt wallet private key failed," + err.Error())
			return nil, err
		}
...
func (w *Wallet) writeAccounts(key []byte, wr *utils.SimpleWriter) {
	...
	for _, pk := range w.accountsKey {
		...
		if err != nil {
			util.Error.Fatal("encrypt wallet private key failed," + err.Error())
		}
...
func (w *Wallet) readHdSeed(key []byte, r *utils.SimpleReader) bool {
	...
	if err != nil {
		util.Error.Println("decrypt wallet hd seed failed," + err.Error())
		return false
	}
	...
	if int(size) > len(decryptBites) {
		util.Error.Println("parse wallet mnemonic failed, bytes length error")
		return false
	}
	w.mnemonicPhrase = string(r2.ReadCString(int(size)))
	r2.ReadInt(binary.BigEndian, &w.nextAccountIndex)
	if r2.Error() != nil {
		util.Error.Println("parse wallet mnemonic failed," + r2.Error().Error())
		return false
	}
	return true
}
...
func (w *Wallet) writeHdSeed(key []byte, wr *utils.SimpleWriter) {
	...
	if wr2.Error() != nil {
		util.Error.Fatal("write wallet hd seed failed," + wr2.Error().Error())
	}
	iv := make([]byte, 16)
	rand.Read(iv)
	writeBytes(iv, wr)
...
// 	if err != nil {
// 		util.Error.Fatal("generate wallet encrypt key failed," + err.Error())
// 	}
// 	w.writeAccounts(key, wr)
// 	w.writeHdSeed(key, wr)
// 	if wr.Error() != nil {
// 		util.Error.Fatal("write wallet to bytes failed," + wr.Error().Error())
// 	}

// 	dir := path.Dir(w.file)
// 	if _, err = os.Stat(dir); os.IsNotExist(err) {
// 		if err = fileutils.MkdirAll(dir); err != nil {
// 			util.Error.Fatal("create wallet dir failed," + err.Error())
// 		}
// 	}
// 	err = fileutils.WriteFile(w.file, wr.BytesUncheck())
// 	if err != nil {
// 		util.Error.Fatal("flush wallet data failed," + err.Error())
// 	}
...
func (w *Wallet) requireUnlocked() {
	if w.password == "" {
		util.Error.Fatal("wallet is locked")
	}
}
...
func (w *Wallet) AddAccountWithNextHdKey() *secp256k1.PrivateKey {
	...
	if !w.IsHdWalletInitialized() {
		util.Error.Fatal("HD Seed is not initialized")
	}
