...
func (s *Server) Run(listen string) error {
	...
	s.logger.Logf("[INFO] listen on %s", listen)
...
func (s *Server) handler(w http.ResponseWriter, r *http.Request) {
	...
	defer func(id uint64) {
		if err1 := recover(); err1 != nil && req.Method == "xdag_updatePoolConfig" {
			render.JSON(w, r, Response{Version: "2.0", ID: id, Result: nil, Error: "password error"})
		}
	}(req.ID)
